...
class PhoneNumberRootComponent(
                ...
                Child.CountryChooser(
                    component = get<CountryChooserComponent> {
                        parametersOf(
                            componentContext,
                            config.countries,
                            { selectedCountry: CountryChooserUiModel.Country? ->
                                val previousChildNavState = childStack.backStack
                                    .lastOrNull { child ->
                                ...
                                if (previousChildNavStateInstance is Child.PhoneNumber) {
                                    val phoneNumberComponent =
                                        previousChildNavStateInstance.component
    ...
    sealed class Config {
        ...
        @Serializable
        data class CountryChooser(val countries: List<CountryChooserUiModel>) : Config()
    }
}
